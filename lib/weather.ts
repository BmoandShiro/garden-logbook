...
export async function getWeatherDataForZip(zip: string) {
  ...
  const response = await axios.get(`https://api.weather.gov/points/${zip}`);
  ...
  const alertsResponse = await axios.get(`https://api.weather.gov/alerts/active?zone=${zip}`);
  const alerts = alertsResponse.data.features;

  // Determine if there are frost or flood alerts
  const hasFrostAlert = alerts.some((alert: any) => alert.properties.event === 'Frost Advisory' || alert.properties.event === 'Freeze Warning');
  const hasFloodAlert = alerts.some((alert: any) => alert.properties.event === 'Flood Warning');
